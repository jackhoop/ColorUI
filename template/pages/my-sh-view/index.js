...
Page({
  ...
  switchChange: function (e) { 
    ...
    if (e.detail.value) { 
      ...
      bz= '店铺审核不通过，请修改正确认后重新提交审核'
    }
    that.setData({
      ["business.sftg"]: e.detail.value,
      ["business.bz"]: bz,
    });
  },
  ...
  bindTextBtgBlur: function (e) { 
    var that = this;
    that.setData({
      ["business.btg"]: e.detail.value,
    });
  },
  bindTextBzBlur: function (e) {
    var that = this;
    that.setData({

      ["business.bz"]: e.detail.value,
    });
  },
  getBusiness() {
    wx.showLoading({
    ...
    wx.request({
      ...
      success: function (res) {
        if (res.data.business) {
          res.data.business.sftg =true
          res.data.business.bz = '店铺审核通过，请尽快添加商品'
          that.setData({
 ...
 */
  ...
  jcchooseImage: function (e) {
    ...
    wx.previewImage({
      ...
      urls: that.data.jcfiles // 需要预览的图片http链接列表  
    })
  },
